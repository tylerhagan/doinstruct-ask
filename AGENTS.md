...
This is the deliverable, not documentation about the deliverable. The claim it
makes is testable: an engineer who has never seen this codebase should be able to
change the flow and the screens with two or three prompts and ship, with a human
doing only final polish. `docs/handover-proof.md` is the receipt.

---

## 1. Why this file is the important artifact

doinstruct's goal is agents producing >95% good UI. In **this** stack that goal
currently fails, and it fails for a specific, diagnosable reason.
...
| ---------- | ------- | ----------------------------------------------- |
| Svelte     | 5.56    | Runes only. `runes: true` is forced in `vite.config.ts` — Svelte 4 syntax is a **compile error**, not a warning. |
| SvelteKit  | 2.63    | `adapter-static`. Everything prerenders. No server, no API routes. |
| Tailwind   | 4.3     | CSS-first. Theme lives in `src/lib/design/tokens.css` under `@theme`. |
...
| `createEventDispatcher`| callback props: `onstart`, `onstop` |
| `<slot />`             | `{@render children()}` + `Snippet` |
| `export let value` + `bind:` | `$bindable()`              |

Two more that are easy to miss:

- Multi-statement derivations use `$derived.by(() => { … })`, not `$derived(() => …)`.
  The latter assigns the *function*, and the bug is silent.
- Reassign to trigger reactivity on collections. `set.add(x)` does not notify;
  `set = new Set([...set, x])` does. See `StepList.svelte`.

**Verify yourself.** `npm run check` must print `0 errors`. Do not report a task
...
- Never write a raw hex value, a raw px size, or an arbitrary value like
...
## 5. Hard rules — these encode safety and accessibility, not taste

Breaking one of these is a defect even if it looks fine.

1. **Nothing interactive is smaller than 64px** (`min-h-tap`). Gloves, not
...
3. **Lime appears exactly once per screen** — the primary voice action. Two lime
   elements means the screen is wrong.
4. **Red (`stop`) is reserved for safety.** Hazards, lockout/tagout, and refusals
   only. Never for validation errors, never for "delete". A worker must be able
   to trust that red means stop working.
...
6. **No user-facing string literals in components.** Add a key to
   ...
   German-only string is a defect, not a placeholder.
7. **Never truncate content.** Wrap. German compounds and Romanian diacritics
...
```
src/lib/design/tokens.json   Source of truth + the REASONING for each divergence
...
**Architectural rule that makes prompting predictable:** components are dumb.
They render props and report gestures; they never own flow state. The state
machine lives only in `+page.svelte`.

That is why "change the flow" touches one file, and "restyle the answer" touches
...
Full contracts are in the header comment of each file. Read that header before
...
| `EscalationCard` | The miss → named human → capture | `escalation`, `onnotify`, `oncapture` |
...
> dominant elements and the explanatory text is secondary. Token classes only,
> no raw values, keep every existing prop and aria attribute. Run `npm run check`.
...
6. Lime still appears at most once per screen.

If you cannot satisfy one of these, say so explicitly rather than working around
it. A silent workaround in a safety-adjacent product is worse than a blocked task.
